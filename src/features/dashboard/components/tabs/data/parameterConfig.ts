...
export const INITIALIZATION_PARAMETERS = [
  { id: "train_ratio", label: "TRAIN_RATIO", placeholder: "e.g., 0.9", unit: "%" },
  { id: "n_paths_pd", label: "N_PATHS_PD", placeholder: "e.g., 100000" },
  { id: "resid_mode_pd", label: "RESID_MODE_PD", placeholder: 'e.g., "normal"' },
...
export const getDefaultParameterValues = (stressColumns: string[] = []) => {
  // Initialize empty objects for dynamic fields
  const exposure: Record<string, string> = {};
  const ead: Record<string, string> = {};
  const npl: Record<string, string> = {};

  // Populate based on stress_columns
  stressColumns.forEach(segment => {
    exposure[segment] = "";
    ead[segment] = "";
    npl[segment] = "";
  });
